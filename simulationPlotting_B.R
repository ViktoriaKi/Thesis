...
plotVals <- function(df, fwer = NA) {
  ...
  plot(as.numeric(rownames(df)), df[,1], type = "o", col = mypal[1], ylim = c(0, 1), xlim = c(0.5, 1),
       xlab = "Selection Fraction", ylab = "Power", main = paste0("Carving Adjusted Power with Different B values"), 
  ...
  legend('topright', legend=legendNames, col = mypal, lty = 1:4, pch = 1:4 + 14, ncol = 2)
  # dev.off()
}
